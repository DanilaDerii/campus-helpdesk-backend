...
import { HttpError } from "./guardrails/errors.js";
import type { ExternalIdentity } from "../integrations/identity/support.js";
import {
  AuthenticationError,
...
import {
  completeMicrosoftLogin as getMicrosoftIdentity,
  createMicrosoftLoginUrl,
  EntraNotConfiguredError,
} from "../integrations/identity/entra.js";
import {
...
const startMicrosoftLogin: RequestHandler = async (_request, response, next) => {
  try {
    ...
    const authorizationUrl = await createMicrosoftLoginUrl(
      transaction.state,
...
const completeMicrosoftLoginHandler: RequestHandler = async (
  request,
  response,
  next,
) => {
  try {
    ...
    verifyLoginState(request.query.state, transaction.state);
    ...
    try {
      identity = await getMicrosoftIdentity({
        code,
        codeVerifier: transaction.codeVerifier,
      });
    } catch (error: unknown) {
...
entraAuthRoutes.get("/callback", completeMicrosoftLoginHandler);
