...
Students report university problems. Technicians claim, comment on, update and
resolve tickets. Administrators manage tickets, users and technician
assignments.

## Main functions

- University Microsoft login, followed by a HelpDesk JWT for API authentication.
- Role-based access control (RBAC) with student, technician and admin roles.
- Ticket creation, listing, detail, assignment, status changes and comments.
...
The four tables are `users`, `ticket_categories`, `tickets` and
`ticket_comments`. See [ERD.md](ERD.md) and [schema.prisma](../prisma/schema.prisma)
